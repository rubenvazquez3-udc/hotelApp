...
public class HotelConversor {
	...
	public final static Hotel toHotel(HotelDto hotelDto) {
		return new Hotel(hotelDto.getName(), hotelDto.getManager(),hotelDto.getAddress());
	}

	public final static HotelDto toHotelDto(Hotel hotel) {
		return new HotelDto(hotel.getId(),hotel.getName(),hotel.getManager(), hotel.getAddress());
	}
}
