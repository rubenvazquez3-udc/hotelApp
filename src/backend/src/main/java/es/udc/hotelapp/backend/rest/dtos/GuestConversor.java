...
public class GuestConversor {
	...
	public final static Guest toGuest(GuestDto guestDto) {
		return new Guest(guestDto.getName(), guestDto.getSurname(), guestDto.getDni(), guestDto.getAddress());
	}

	public final static GuestDto toGuestDto(Guest guest) {
		return new GuestDto(guest.getId(),guest.getName(),guest.getSurname(),guest.getDni(), guest.getAddress());
	}
}
