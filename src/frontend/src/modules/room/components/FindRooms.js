...
const FindRooms = () =>{
    ...
    const hotel1 = hotels.filter(hotel.address === user.address); //Comprobar que fai realmente o filter e o acceso sea correcto

    const handleSubmit = event => {
        event.preventDefault();

         dispatch(actions.findRooms(hotel1.id, status.trim()));
